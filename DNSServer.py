...
# --- GLOBAL CONSTANTS (Important for consistency) ---
# A fixed, persistent salt is necessary for the key generation to be deterministic
# and allow for successful decryption.
# For a real application, this would be stored securely.
SALT = os.urandom(16)
...
# --- Cryptography Execution (for testing) ---
...
dns_records: Dict[str, Dict[dns.rdatatype.RdataType, Any]] = {
    'example.com.': {
        ...
        # dns.rdatatype.CNAME: 'www.example.com.', # CNAME should not exist with A/AAAA
    ...
    # Added the NYU record which appears in the tests
    'nyu.edu.': {
        dns.rdatatype.A: '128.122.100.100',
        dns.rdatatype.TXT: ('NYU is a great school!',),
    }
}


def run_dns_server():
    # Create a UDP socket and bind it to the local IP address (0.0.0.0) and port (53)
    ...
    while True:
        try:
            ...
            # For simplicity, we process the first question
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    ...
                    # TXT records need to be enclosed in double quotes when using from_text
                    rdata_list = [dns.rdata.from_text(dns.rdataclass.IN, qtype, f'"{data}"') for data in answer_data]
                else:
                    # Handles A, AAAA, NS, and other single-string records
                ...
                # FIX for unknown domain/record type: Set the RCODE to NXDOMAIN (3)
                # This fixes the "Incorrect error response" issue.
            ...
            print(f"Responding to request: {qname} type: {dns.rdatatype.to_text(qtype)} -> {addr}")
            ...
            # Catch exceptions and print them for debugging
