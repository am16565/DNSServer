...
# --- Configuration ---
# Set the port to 5353 to avoid conflicts with the system DNS service (port 53)
DNS_PORT = 5353
DNS_IP = '127.0.0.1'
...
password = 'your.email@nyu.edu'
input_string = 'AlwaysWatching'

encrypted_value = encrypt_with_aes(input_string, password, salt)
# Convert the byte object to a string for storage in the TXT record
...
def run_dns_server():
    # Create a UDP socket and bind it to the local IP address (127.0.0.1) and port (5353)
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    # BIND: Running with elevated privileges (Run as Administrator) should resolve the WinError 10013
        ...
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                for rdata in rdata_list:
                    # Create an RRset and add the rdata
...
def run_dns_server_user():
    ...
    print(f"DNS server is running on {DNS_IP}:{DNS_PORT}...")
